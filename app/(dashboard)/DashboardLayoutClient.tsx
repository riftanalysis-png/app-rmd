...
export default function DashboardLayoutClient({
  ...
  const [userPhoto] = useState(initialUserPhoto);
  const [teamLogo] = useState(initialTeamLogo);
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState(() => {
    ...
    const savedState = window.localStorage.getItem('rmd_sidebar_state');
  ...
  const toggleSidebar = () => {
    ...
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('rmd_sidebar_state', JSON.stringify(newState));
    }
  };
...
function SidebarItem({ title, subtitle, isAction = false, href, isActive = false, icon: Icon = FolderIcon, isOpen = true }: SidebarItemProps) {
  const content = (
