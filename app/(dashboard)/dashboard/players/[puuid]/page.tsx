...
const DEFAULT_AVATAR = "https://api.dicebear.com/7.x/avataaars/svg?seed=operative";
...
export default function PlayerProfilePage() {
  ...
  const fetchTacticalData = useCallback(async () => {
    ...
    try {
      ...
      if (pRes.data && allPRes.data) {
        ...
        const { data: pMatches } = await supabase.from('player_stats_detailed').select('*').eq('puuid', puuid).order('match_id', { ascending: true });
        
        if (pMatches && pMatches.length > 0) {
          const matchIds = pMatches.map(m => m.match_id);
          
          if (matchIds.length > 0) {
            const { data: participants } = await supabase.from('player_stats_detailed').select('match_id, puuid').in('match_id', matchIds);
            
            const enriched = pMatches.map(m => {
              ...
              const opponent = matchParticipants.find(p => { 
                ...
                return t && t.toLowerCase() !== myTeam.toLowerCase(); 
              });
              
              return { 
                ...m, 
                opponent_acronym: opponent ? teamLookup[opponent.puuid] : 'MIX', 
                is_win: m.win === true || String(m.win).toLowerCase() === 'win', 
      ...
      if (allPRes.data) setAllPlayers(allPRes.data);
  ...
  // --- LOGICA DE FILTRAGEM GLOBAL ---
  const filteredMatches = useMemo(() => {
    if (sideFilter === 'ALL') return matches;
    return matches.filter(m => m.side?.toUpperCase() === sideFilter);
  }, [matches, sideFilter]);

  // --- MATEMÁTICA ANALÍTICA BASEADA NO FILTRO ---
  ...
  if (loading) return <div className="p-20 text-blue-500 font-black text-center animate-pulse italic uppercase tracking-[0.2em]">Circuito Desafiante: Syncing System...</div>;
  if (!player) return <div className="p-20 text-red-500 font-black text-center uppercase tracking-widest">Operativo não encontrado no banco de dados.</div>;

  return (
    <div className="p-6 max-w-[1700px] mx-auto space-y-8 bg-[#06090f] min-h-screen text-white font-black uppercase italic tracking-tighter relative">
      ...
      {showGuide && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[300] p-12 overflow-y-auto animate-in fade-in duration-500">
           <button onClick={() => setShowGuide(false)} className="absolute top-8 right-8 text-slate-500 hover:text-white transition-colors text-4xl">×</button>
           <div className="max-w-4xl mx-auto space-y-12">
              <header className="border-l-4 border-blue-500 pl-6"><h1 className="text-5xl font-black italic">INTEL MANUAL: PROTOCOLO TIER 2</h1><p className="text-blue-400 tracking-[0.3em]">GUIA DE FILTRAGEM POR LADO</p></header>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-[11px] leading-relaxed tracking-wider">
                <GuideSection title="Filtro de Lado" text="BLUE/RED recalcula todos os gráficos. Essencial para analisar se o jogador mantém o mesmo nível de agressividade sem o auxílio do counter-pick no Red Side." />
                <GuideSection title="R² e Side" text="Veja como a previsibilidade do operativo muda. Jogadores mais instáveis costumam ter R² menor no lado em que o time tem winrate mais baixo." />
                <GuideSection title="Eficiência" text="O tamanho da bolha reflete a Gold Efficiency. Compare se o jogador converte melhor o ouro jogando no topo (Blue) ou na base (Red) do mapa." />
                <GuideSection title="BoxPlot" text="As caixas verticais mostram a consistência de Gold/XP. Observe se as caixas do Red Side são mais longas (indicando instabilidade)." />
              </div>
           </div>
        </div>
      )}

      {/* TOP NAV COM FILTRO DE SIDE - CORRIGIDO O Z-INDEX PARA 200 */}
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6 bg-slate-900/40 p-4 rounded-[32px] border border-slate-800 shadow-2xl backdrop-blur-md relative z-[200]">
        <div className="flex items-center gap-6" ref={dropdownRef}>
          <div className="flex gap-5 px-4 border-r border-slate-800">
            {teams.map((t: any) => (
              <button key={t.acronym} onClick={() => setActiveDropdown(activeDropdown === t.acronym ? null : t.acronym)} className={`transition-all hover:scale-110 outline-none relative`}>
                <img src={t.logo_url} className={`w-10 h-10 object-contain transition-all ${t.acronym === player.team_acronym || activeDropdown === t.acronym ? 'opacity-100 scale-125' : 'opacity-20 hover:opacity-100'}`} alt={t.acronym} />
                {activeDropdown === t.acronym && (
                  // CORRIGIDO O Z-INDEX PARA 210
                  <div className="absolute top-full left-0 mt-4 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl min-w-[200px] overflow-hidden z-[210]">
                    {allPlayers.filter((p:any) => p.team_acronym === t.acronym).map((tm:any) => (
                      <button key={tm.puuid} onClick={() => { router.push(`/dashboard/players/${tm.puuid}`); setActiveDropdown(null); }} className="w-full text-left px-4 py-3 text-[10px] hover:bg-blue-600 border-b border-slate-800 last:border-0">{tm.nickname}</button>
                    ))}
                  </div>
                )}
              </button>
          ...
          <div className="flex bg-slate-950/80 p-1 rounded-2xl border border-slate-800">
            <button onClick={() => setSideFilter('ALL')} className={`px-4 py-1.5 rounded-xl text-[9px] transition-all ${sideFilter === 'ALL' ? 'bg-slate-800 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>BOTH</button>
            <button onClick={() => setSideFilter('BLUE')} className={`px-4 py-1.5 rounded-xl text-[9px] transition-all ${sideFilter === 'BLUE' ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : 'text-blue-900 hover:text-blue-400'}`}>BLUE SIDE</button>
            <button onClick={() => setSideFilter('RED')} className={`px-4 py-1.5 rounded-xl text-[9px] transition-all ${sideFilter === 'RED' ? 'bg-red-600 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)]' : 'text-red-900 hover:text-red-400'}`}>RED SIDE</button>
          </div>
        </div>
        <input type="text" placeholder="PESQUISAR OPERATIVO..." className="w-64 bg-slate-950 border border-slate-800 rounded-2xl px-5 py-2.5 text-[10px] outline-none shadow-inner focus:border-blue-500/50 transition-all" />
      </div>

      <div className="grid grid-cols-12 gap-6 items-stretch">
        
        {/* CARTINHA: ESTILO SCOUTING REPORT */}
        <div className="col-span-12 xl:col-span-3 flex flex-col gap-6 z-[150] sticky top-6 h-fit">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-[32px] p-6 relative overflow-hidden shadow-2xl backdrop-blur-md">
            ...
            <p className="absolute top-3 right-5 text-[8px] text-slate-500 font-mono tracking-widest uppercase">ID: {(player.puuid || "00000000").substring(0,8)}</p>
              ...
              <div className="relative group">
                ...
                <div className="relative w-36 h-36 rounded-2xl border border-slate-700 bg-slate-950 p-1.5 shadow-inner overflow-visible">
                  ...
                  <div className="absolute -bottom-4 -right-4 bg-[#06090f] border border-slate-700 p-2.5 rounded-xl shadow-xl z-20 group-hover:border-blue-500/50 transition-colors">
                    <img src={getRoleIcon(player.primary_role)} className="w-6 h-6 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]" alt={player.primary_role} />
                  </div>
                </div>
              </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-[32px] overflow-hidden shadow-2xl backdrop-blur-md">
            <div className="bg-slate-950/80 py-3 px-6 border-b border-slate-800 flex justify-between items-center">
              <p className="text-[9px] text-slate-400 tracking-[0.3em] uppercase">Avaliação de Desempenho</p>
              <span className="text-[7px] font-mono text-blue-500 bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20">CONFIDENTIAL</span>
            </div>
            {/* Grid 2x2 sem bordas arredondadas internas, divididas por uma linha (gap-px) */}
            <div className="grid grid-cols-2 gap-px bg-slate-800">
              <StatBadge label="LANE" val={player.median_lane} />
              <StatBadge label="IMPACTO" val={player.median_impact} />
              <StatBadge label="CONV" val={player.median_conversion} />
              <StatBadge label="VISION" val={player.median_vision} />
            </div>
          </div>
        ...
        <div className="col-span-12 xl:col-span-9 space-y-8">
          ...
          <div className="bg-slate-900/10 border border-slate-800/40 rounded-[48px] p-8 shadow-inner">
            <h3 className="text-blue-500 text-[10px] tracking-[0.5em] mb-10 text-center font-black italic uppercase">Matriz Relacional de Performance ({sideFilter})</h3>
